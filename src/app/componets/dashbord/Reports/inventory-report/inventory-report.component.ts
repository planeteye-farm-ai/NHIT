...
export class InventoryReportComponent {
  ...
  filterDistress() {
    ...
    const formAssetType = this.filterForm.get('asset_type')?.value || [];
    ...
    let dataObj = {
      ...
      asset_type: Array.isArray(formAssetType)
        ? formAssetType
        : formAssetType
        ? [formAssetType]
        : [],
    ...
    this.inventoryArray.forEach((records: any[]) => {
      records.forEach((row) => {
        ...
        if (
          ...
          if (
            inventoryName == 'Crash Barrier' ||
            inventoryName == 'crash_barrier' ||
            inventoryName == 'Service Road' ||
            inventoryName == 'service_road'
          ) {
