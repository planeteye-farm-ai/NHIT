...
export class EditRigidDistressComponent {
  ...
  async onSubmit(): Promise<void> {
    ...
    const index = testRigidDistress.findIndex(
      (d: any) => d.rigid_distress_id === this.distressId
    );

    if (index !== -1) {
      testRigidDistress[index] = {
        ...testRigidDistress[index],
        ...formValue,
        rigid_distress_id: this.distressId,
    ...
    if (apiSuccess) {
      this.toastr.success('Rigid distress updated successfully!', 'Success', {
        timeOut: 3000,
        positionClass: 'toast-top-right',
      });
