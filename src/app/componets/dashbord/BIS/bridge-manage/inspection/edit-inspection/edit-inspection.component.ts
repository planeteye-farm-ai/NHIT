...
import { BridgeInspection,BridgeInspectionUpdate,Suggestion } from '../inspection';
...
export class EditInspectionComponent {
  ...
  constructor(
    ...
    ngOnInit(): void {
      ...
      this.addInspectionItem1('Cracks In Deck Slab And Leakages');
      this.addInspectionItem1('I Beam Leakage');
      this.addInspectionItem1('Expansion Joints');
      this.addInspectionItem1('Bearings');
      this.addInspectionItem1('Drainage Pipe');
  ...
  onSubmit(): void {
    ...
    let inspection:BridgeInspectionUpdate ={ 
  ...
  addInspectionItem1(itemName: string) {
    const itemGroup = this.fb.group({
      items_needing_attentions: [itemName],
      actions_recommended: [''],
      suggestion_time: [''],
      remarks: ['']
    });
  ...
  addInspectionItem(inspectionSugession:any) {
    ...
    inspectionSugession.forEach((suggestion: any) => {
      this.inspectionItems.push(
        this.fb.group({
          items_needing_attentions: [suggestion.items_needing_attentions],
          actions_recommended: [suggestion.actions_recommended],
          suggestion_time: [suggestion.suggestion_time],
  ...
  patchValue(inspection:any){
    ...
    if(inspection.data.suggestions.length !=0 )
    {
  ...
  onFileChange(event: Event, controlName: string): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      formData.append('inspection_id',this.inspectionId);
      formData.append('inspection_image',file);
      formData.append('field_name',controlName);
      console.log(formData)
      this.bridgeService.addParticularImage(formData).subscribe((res)=>{
        // console.log(res);
        if(res.status){
          this.ngOnInit()
        }
  ...
  delete(filedName:any){
    ...
    this.bridgeService.deleteInspectionImage(formData).subscribe((res)=>{
      // console.log(res);
      if(res.status){
        this.ngOnInit()
      }
