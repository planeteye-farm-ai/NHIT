...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [RouterModule,NgbModule,FormsModule,ReactiveFormsModule,ToastrModule,HttpClientModule
],
  ...
  this.adminLogin(loginData).subscribe({
    next: (res) => {
      if (res.status === true) {
        ...
        localStorage.setItem('name', `${res.data[0].first_name} ${res.data[0].last_name}`);
